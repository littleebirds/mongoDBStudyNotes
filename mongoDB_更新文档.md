...
### 更新操作符
   ...
   + 数组更新操作符
      1. $:占位更新
      2. $addToSet：只有元素不存在则添加元素到指定的数组里
      3. $pop：移除数组里第一个或者最后一个元素
      4. $pullAll:移除数组里所有值
      5. $pull：移除数组里所有匹配到的值
      6. $pushAll：添加若干个元素到数组里
      7. $push：添加一个元素到数组里
      8. $each：与addToSet或者$push搭配使用，对目标数组做遍历
      9. $slice:与$push搭配使用，对添加的元素做限制，如果为负数，则截取目标元素，如果为整数则只显示slice指定的数目
      10. $sort：与$push、$each搭配 对目标数组进行排序。1表示升序，-1表示降序
      11. $position：与$push，$each搭配 ，确定在哪个位置插入数组
   + 按位操作符
